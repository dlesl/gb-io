...
fn origin<'a>(len: Option<usize>) -> impl FnMut(&'a [u8]) -> IResult<&'a [u8], (Option<String>, Vec<u8>)> {
    separated_pair(
        origin_tag,
        space1,
        sequence(len)
...
fn sequence_chunk(i: &[u8]) -> IResult<&[u8], &[u8]> {
    delimited(opt(pair(digit1, space1)), alpha1, multispace1)(i)
}

fn sequence<'a>(len: Option<usize>) -> impl FnMut(&'a [u8]) -> IResult<&'a [u8], Vec<u8>> {
    map_res(
...
fn gb(i: &[u8]) -> IResult<&[u8], Seq> {
    let (i, locus) =  locus(i)?;
    let (i, metadata) =  map_res(fields, |f| fill_seq_fields(Seq::empty(), f))(i)?;
    let (i, _) = many0(ignored_line)(i)?;
    let (i, features) =  opt(features)(i)?;
    let (i, _) = opt(base_count)(i)?;
    let (i, contig) =  opt(contig_text)(i)?;
    let (i, origin) =  opt(origin(locus.len))(i)?;
    let (i, _) = multispace0(i)?;
    let (i, _) = tag("//")(i)?;
    let (i, _) = opt(complete(multispace1))(i)?;
    Ok((i, Seq {
        name: locus.name,
        date: locus.date,
...
pub fn gb_records(i: &[u8]) -> IResult<&[u8], Vec<Seq>> {
    preceded(skip_preamble, many1(gb))(i)
}